        ...
        >
          <View style={[styles.header, { borderBottomColor: colors.border }]}>
            <Text style={[styles.headerTitle, { color: colors.textPrimary }]}>
              {isEditing ? t('quickActionForm.editTitle') : t('quickActionForm.newTitle')}
            </Text>
          ...
          >
            <TextField
              label={t('quickActionForm.titleLabel')}
              required
              placeholder={t('quickActionForm.titlePlaceholder')}
              value={title}
              onChangeText={(v) => setTitle(v.slice(0, MAX_TITLE_LENGTH))}
              maxLength={MAX_TITLE_LENGTH}
              accessibilityLabel={t('quickActionForm.titleLabel')}
            />

            <TextField
              label={t('quickActionForm.descriptionLabel')}
              placeholder={t('quickActionForm.descriptionPlaceholder')}
              value={description}
              onChangeText={(v) => setDescription(v.slice(0, MAX_DESCRIPTION_LENGTH))}
              maxLength={MAX_DESCRIPTION_LENGTH}
              accessibilityLabel={t('quickActionForm.descriptionLabel')}
            />

            <SelectField
              label={t('quickActionForm.actionTypeLabel')}
              required
              value={t(`quickActionForm.actionTypes.${actionType}`)}
              placeholder={t('quickActionForm.actionTypePlaceholder')}
              options={[...ACTION_TYPE_OPTIONS]}
              getOptionLabel={(opt) => t(`quickActionForm.actionTypes.${opt}`)}
              optionToValue={(opt) => opt}
              onChange={(val) => setActionType(val as QuickActionType)}
              searchable={false}
              accessibilityLabel={t('quickActionForm.actionTypeLabel')}
            />

            {actionType === 'navigate' ? (
              <SelectField
                label={t('quickActionForm.destinationLabel')}
                required
                value={(() => {
                  const key = NAVIGABLE_ROUTE_KEYS.find((r) => r.value === route)?.labelKey;
                  return key ? t(`quickActionForm.routes.${key}`) : '';
                })()}
                placeholder={t('quickActionForm.destinationPlaceholder')}
                options={NAVIGABLE_ROUTE_VALUES}
                getOptionLabel={(opt) => {
                  const key = NAVIGABLE_ROUTE_KEYS.find((r) => r.value === opt)?.labelKey;
                  return key ? t(`quickActionForm.routes.${key}`) : opt;
                }}
                optionToValue={(opt) => opt}
                onChange={setRoute}
                searchable={false}
                accessibilityLabel={t('quickActionForm.destinationLabel')}
              />
            ) : null}

            {actionType === 'chat-prompt' ? (
              <TextField
                label={t('quickActionForm.chatPromptLabel')}
                required
                placeholder={t('quickActionForm.chatPromptPlaceholder')}
                value={chatPrompt}
                onChangeText={(v) => setChatPrompt(v.slice(0, MAX_PROMPT_LENGTH))}
                maxLength={MAX_PROMPT_LENGTH}
                multiline
                numberOfLines={3}
                accessibilityLabel={t('quickActionForm.chatPromptLabel')}
            ...
            <View style={styles.buttonRow}>
              <Button label={t('common:save')} onPress={handleSave} disabled={!canSave} />
            </View>
          </ScrollView>
        </Animated.View>
      </View>
